# ===== PRUEBA GOOGLE API / THINKER FOTOS=====
...
# Detectar base_dir correctamente
base_dir = Path(__file__).parent if '__file__' in globals() else Path.cwd()
image_path = base_dir.parent / 'public' / 'images' / 'fondo2.png'

try:
    imagen_fondo_original = Image.open(image_path)
    print(f"Imagen de fondo cargada desde: {image_path}")
...
def redimensionar(event=None):
    ...
    if not imagen_mostrada and service_global and nombre_img_global:
        esperar_recuadro_y_mostrar(nombre_img_global, service_global)
...
def obtener_id_imagen_por_nombre(nombre_archivo, service):
    try:
        ...
        if items:
            print(f"Archivo '{nombre_archivo}' encontrado. ID: {items[0]['id']}")
            return items[0]['id']
        else:
            print(f"No se encontró '{nombre_archivo}' en Drive.")
            return None
    except Exception as e:
...
def esperar_recuadro_y_mostrar(nombre_imagen, service):
    global imagen_mostrada
    if imagen_mostrada:
        return
    recuadro_w = recuadro_video.winfo_width()
    recuadro_h = recuadro_video.winfo_height()
    if recuadro_w < 50 or recuadro_h < 50:
        print("Esperando tamaño del recuadro...")
        root.after(300, lambda: esperar_recuadro_y_mostrar(nombre_imagen, service))
        return
    mostrar_imagen_en_recuadro(nombre_imagen, service)

# ------------------ MAIN ------------------

if __name__ == '__main__':
    service_global = autenticar_drive()
    nombre_img_global = input("Nombre exacto de la imagen en Drive: ").strip()
